...
def sync_with_pinecone(data):
    ...
    for i in tqdm(range(0,len(data),batch_size),desc="Processing Batches",unit='batch',total=total_batches):
        ...
        texts=[
            f"{row['ProductDescription']} {row['ProductName']} {row['ProductBrand']} {row['Gender']} {row['Price']} {row['PrimaryColor']}"
...
########################################################       
    # embed texts
        ...
        # Wait to avoid Gemini free-tier rate limits
        time.sleep(15)
######################################################
